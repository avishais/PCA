...
// #define KDL_ABB "../ckc/kdl_class.h"
// #define KDL_BAXTER "kdl_class.h"
...
class StateValidityChecker : public collisionDetection, public kdl, public stats::pca
{
public:
	/** Constructors */
	StateValidityChecker(const ob::SpaceInformationPtr &si, int dimension) :
		mysi_(si.get()),
		stats::pca(dimension),
		n_(dimension)
			{q_prev.resize(dimension);
			set_do_bootstrap(false, 100);
			}; //Constructor
	StateValidityChecker(int dimension) :
		stats::pca(dimension),
		n_(dimension)
			{q_prev.resize(dimension);
			set_do_bootstrap(false, 100);
