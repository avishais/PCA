...
pca::pca(long num_vars, int num_records_knn)
	: num_vars_(num_vars),
	  num_records_(0),
	  record_buffer_(num_records_knn),
	  solver_("dc"),
	  do_normalize_(false),
	  do_bootstrap_(false),
	  num_bootstraps_(10),
...
void pca::clear_records() {

	if (data_.n_rows == record_buffer_) 
		data_.zeros();
	else
		data_.zeros(record_buffer_,num_vars_);
	num_records_ = 0;
		
}

void pca::print_records() {
...
void pca::solve() {
	...
	if (data_.n_rows > num_records_) // Addition for efficiency (not using a buffer but a constant size) - Avishai
	...
	//eigval_ *= 1./energy_(0); // *** Canceled by Avishai Sintov to match the generic PCA results (such as in Matlab)
