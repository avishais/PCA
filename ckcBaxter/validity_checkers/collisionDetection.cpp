...
// Returns 0 if no collision
{
	...
	if(withObs) {
		...
		PQP_Tolerance(&res[104],Rtable,Ttable,&table,REE,TEE,&EE,tolerance);
		PQP_Tolerance(&res[105],Rtable,Ttable,&table,R5,T5,&link5,tolerance);
		PQP_Tolerance(&res[106],Rtable,Ttable,&table,R6,T6,&link6,tolerance);
		PQP_Tolerance(&res[107],Rtable,Ttable,&table,R7,T7,&link7,tolerance);
		PQP_Tolerance(&res[108],Rtable,Ttable,&table,REE2,TEE2,&EE2,tolerance);
		PQP_Tolerance(&res[109],Rtable,Ttable,&table,R52,T52,&link52,tolerance);
		PQP_Tolerance(&res[110],Rtable,Ttable,&table,R62,T62,&link62,tolerance);
		PQP_Tolerance(&res[111],Rtable,Ttable,&table,R72,T72,&link72,tolerance);
		PQP_Tolerance(&res[112],Rtable,Ttable,&table,Rrod_v,Trod,&rod,tolerance);

		// Obs1
		// MRotZ(Rcone,0);
		// Tcone[0] = 735; Tcone[1] = -300; Tcone[2] = 20;

		// PQP_Tolerance(&res[113],Rcone,Tcone,&obs,REE,TEE,&EE,tolerance);
		// PQP_Tolerance(&res[114],Rcone,Tcone,&obs,R5,T5,&link5,tolerance);
		// PQP_Tolerance(&res[115],Rcone,Tcone,&obs,R6,T6,&link6,tolerance);
		// PQP_Tolerance(&res[116],Rcone,Tcone,&obs,R7,T7,&link7,tolerance);
		// PQP_Tolerance(&res[117],Rcone,Tcone,&obs,REE2,TEE2,&EE2,tolerance);
		// PQP_Tolerance(&res[118],Rcone,Tcone,&obs,R52,T52,&link52,tolerance);
		// PQP_Tolerance(&res[119],Rcone,Tcone,&obs,R62,T62,&link62,tolerance);
		// PQP_Tolerance(&res[120],Rcone,Tcone,&obs,R72,T72,&link72,tolerance);
		// PQP_Tolerance(&res[121],Rcone,Tcone,&obs,Rrod_v,Trod,&rod,tolerance);
