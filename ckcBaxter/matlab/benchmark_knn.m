...
switch plannerType
    case 'CBiRRT'
        D = load('Benchmark_CBiRRT_dimpca.txt');
...
xlabel('number of eigenvectors for reconstruction');%'k-nearest neighbors');
legend('w/ PCA','w/o PCA');

%%
fprintf('%d results for each k.\n', floor(size(D,1)/length(r)));
fprintf('k = %.1f provides the minimum runtime of: %.2f sec.\n', r(im), tmin);
